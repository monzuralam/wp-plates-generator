function AttUpload(){
	(function($) {
		...
		jQuery('.att-opts-upload').click(function() {
		...
		window.send_to_editor = function(html) {
			if (header_clicked) {
				...
				jQuery(item_clicked).siblings('.att-opts-upload-remove').fadeIn('slow');
		...
		jQuery('.att-opts-upload-remove').click(function(){
			jQuery(this).siblings('input').val('');
			jQuery(this).prev().fadeIn('slow');
			jQuery(this).prev().prev().fadeOut('slow', function(){jQuery(this).attr("src", att_upload.url);});
